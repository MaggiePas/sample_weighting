...
def evaluate_last_timestep(model=None, val_loader=None, device=None, criterion=None, lookup=None, weight=None, basis=None, centering=None, jtt=None):
    subj_id_list = []
    y_pred_list = []
    y_pred_auc = []
    y_pred_list_ge = []
        ...
        if len(next(iter(val_loader))) == 2 or len(next(iter(val_loader))) == 3:
            for local_batch, local_labels, ID in val_loader:
                ...
                if basis is not None:
                    ...
                    input_weight = centering + weight @ basis[lookup[ID[0].item()]]
    ...
    for epoch in range(max_epochs):
        ...
        for local_batch, local_labels, ID in train_loader:
            ...
            if w[counter-1] > median:
                ...
                counter_le += 1
                avg_acc_le += acc.item()
        result = evaluate_last_timestep(model=model, val_loader=val_loader, device=device, criterion=criterion, jtt=None)
...
def train_gru(model=None, criterion=None, optimizer=None, max_epochs=30, train_loader=None, val_loader=None, device=None, seq2seq=True, params=None, batch_size=None, scheduler=None, jtt=None):
            ...
            if counter % 1000 == 0:
                print(
                    "Epoch {}... Step: {}/{}... Average Loss for Epoch: {}... Accuracy: {}".format(epoch, counter, len(train_loader),
                                                                                         avg_loss / counter, avg_acc / counter))
                if seq2seq:
                    evaluate_all_timesteps_per_subject(model=model, val_loader=val_loader, hidden=h, device=device, criterion=criterion, jtt=None)
                else:
                    evaluate_last_timestep(model=model, val_loader=val_loader, device=device, criterion=criterion, jtt=None)
    ...
    if jtt is not None:
        result_train = evaluate_last_timestep(model=model, val_loader=train_loader, device=device, criterion=criterion, jtt=jtt)
    ...
    for epoch in range(max_epochs):
        ...
        for local_batch, local_labels, ID in train_loader:
            ...
            if mode == 1:
                ...
                # Quick debug - no weights or regulatizer in the loss
                # loss = criterion(out.squeeze().to(device),local_labels.squeeze().float().to(device))
        ...
        result = evaluate_last_timestep(model=model, val_loader=val_loader, device=device, criterion=criterion, lookup=lookup_val, weight=weight, basis=basis_val, centering=centering, jtt=None)
