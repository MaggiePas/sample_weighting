...
class GRUNet(nn.Module):
    ...
    def __init__(self, feature_dim, input_dim, hidden_dim, output_dim, n_layers, seq2seq, device, drop_prob=0.0,):
        ...
        # self.gru = nn.Linear(feature_dim, hidden_dim)
...
class GRUSpectralNet(nn.Module):

    def __init__(self, feature_dim, input_dim, hidden_dim, output_dim, n_layers, seq2seq, device, eigen_basis, drop_prob=0.0):
        super(GRUSpectralNet, self).__init__()
        self.hidden_dim = hidden_dim
        self.n_layers = n_layers
        self.seq2seq = seq2seq
        if self.seq2seq:
        ...
        self.dropout = nn.Dropout(drop_prob)
        self.weights = nn.Linear(eigen_basis.shape[1], 1, bias=False)
        self.eigen_basis = eigen_basis
        self.device = device

    def forward_with_loss(self, x, h, local_labels, idx):
        ...
        weight = self.weights(self.eigen_basis[idx])
        criterion = nn.BCEWithLogitsLoss(weight=weight)
        loss = criterion(out.squeeze().to(self.device), local_labels.squeeze().float().to(self.device))
        return out, h, loss
