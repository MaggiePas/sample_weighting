...
def remove_below_18(in_data=None):

    in_data = in_data.loc[in_data['visit_age'] >= 18]

    return in_data

def remove_above_18(in_data=None):

    in_data = in_data.loc[in_data['visit_age'] >= 18]

    return in_data

def remove_22(in_data=None):

    in_data = in_data.loc[in_data['visit_age'] < 22]

    return in_data

def remove_moderate_and_empty(in_data=None, column_name='cahalan_score'):

    in_data = in_data.loc[in_data[column_name] != 'moderate']
    in_data = in_data.dropna(subset=[column_name])
    return in_data

def make_cahalan_binary(in_data=None, column_name='cahalan_score'):
    in_data[column_name] = in_data[column_name].replace('heavy_with_binging', 1)
    in_data[column_name] = in_data[column_name].replace('heavy', 1)
    in_data[column_name] = in_data[column_name].replace('control', 0)
    in_data[column_name] = in_data[column_name].replace('moderate', 0)

    return in_data

def remove_non_control(in_data=None, column_name='cahalan_score'):
    input_feats = in_data[['subject', column_name]]
    input_feats = input_feats.groupby(['subject'])[column_name].max()
    input_feats = pd.DataFrame({'subject':input_feats.index, column_name:input_feats.values})
    subjects = input_feats.loc[input_feats[column_name] == 0]['subject']
    already_binging = in_data[~in_data['subject'].isin(subjects.values)]
    in_data = in_data[in_data['subject'].isin(subjects.values)]
    return in_data, already_binging

def sample_test_set(data, n):
    # find the number of positive sample needed
    proportion = n / len(np.unique(data['subject']))
    num_pos = round(proportion * len(np.unique(data.loc[data['cahalan_score_y'] == 1]['subject'])))
    num_neg = n - num_pos

    pool_pos = data.loc[data['cahalan_score_y'] == 1]['subject']
    pool_neg = data.loc[data['cahalan_score_y'] == 0]['subject']

    set_pos = data.loc[data['subject'].isin(list(pool_pos.sample(num_pos, random_state=100)))]
    set_neg = data.loc[data['subject'].isin(list(pool_neg.sample(num_neg, random_state=100)))]
    # sample function shuffles the rows
    test_set = pd.concat([set_pos, set_neg]).sample(frac=1)
    rest = data.loc[~data['subject'].isin(list(test_set['subject']))]
    return test_set, rest
